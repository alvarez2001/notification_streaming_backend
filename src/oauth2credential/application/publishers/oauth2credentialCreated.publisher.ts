...
import { Oauth2credentialCreatedConsumer } from '../consumers/oauth2credentialCreated.consumer';

@Injectable()
export class Oauth2credentialCreatedPublisher {
    constructor(private readonly rabbitMQService: RabbitMQService) {
        appEvents.on(DeclarationQueues.oauth2credential_created, async oauth2credential => {
    ...
    async send(data: Oauth2credential): Promise<void> {
        const model = {
            ...
            displayNames: {
                [DeclarationQueues.oauth2credential_created]: Oauth2credentialCreatedConsumer.name,
            },
        };
